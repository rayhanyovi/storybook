...
import { login, getMe, completeOnboarding, resetDemoState } from './auth.service.js';
...
export const resetDemoStateHandler: RequestHandler = async (req, res, next) => {
  try {
    const result = await resetDemoState(req.user.sub);
    res.json(result);
  } catch (err) {
    next(err);
  }
